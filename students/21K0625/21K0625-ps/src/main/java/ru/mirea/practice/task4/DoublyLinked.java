...
public class DoublyLinked<S> {
    private ListNode<S> front;
    private int size;
    ...
    public void compare(S string) {
        ListNode<S> current = front; //буферный узел
        ...
        for (int i = -1; i < string.toString().length(); ) { //цикл, определяющий, куда, относительно первого элемента необходимо поставить строку
            for (int j = 0; j < current.data.toString().length(); j++) {
                i++;
                if (string.toString().charAt(i) < current.data.toString().charAt(j)
                        | (string.toString().charAt(i) == current.data.toString().charAt(j)
                                & string.toString().length() < current.data.toString().length()
                                & i == string.toString().length() - 1)) {
                    bool = true;
                    s = 1;
                    break;
                        ...
                        | (string.toString().charAt(i) == current.data.toString().charAt(j)
                        & string.toString().length() > current.data.toString().length()
                        & j == current.data.toString().length() - 1)) {
                    s = 1;
                    break;
                }
            }
    ...
    public void addAfter(S x, S y) {
        if (isEmpty()) {
            System.out.println("Элемент " + x.toString() + " не найден");
        } else {
            ListNode<S> current = front;
            ...
            if (current == null) {
                ...
                ListNode<S> newNode = new ListNode<>(current, y, current.next);
    ...
    public void addBefore(S x, S y) {
        if (isEmpty()) {
            ...
            if (current == null) {
                ...
                ListNode<S> newNode = new ListNode<>(current.prev, y, current);
